...
contract("HulkMaker", ([alice, bar, minter]) => {
  beforeEach(async () => {
    this.factory = await UniswapV2Factory.new(alice, {
      from: alice
    });
    this.hulk = await HulkToken.new({
      from: alice
    });
    await this.hulk.mint(minter, "100000000", {
      from: alice
    });
    ...
    this.maker = await HulkMaker.new(
      this.factory.address,
      bar,
      this.hulk.address,
      this.weth.address
    );
    this.hulkWETH = await UniswapV2Pair.at(
      (await this.factory.createPair(this.weth.address, this.hulk.address))
      .logs[0].args.pair
    );
  ...
  it("should make Hulks successfully", async () => {
    await this.factory.setFeeTo(this.maker.address, {
      from: alice
    });
    await this.weth.transfer(this.hulkWETH.address, "10000000", {
      from: minter,
    });
    await this.hulk.transfer(this.hulkWETH.address, "10000000", {
      from: minter,
    });
    await this.hulkWETH.mint(minter);
    ...
    // After calling convert, bar should have Hulk value at ~1/6 of revenue
    await this.maker.convert(this.token1.address, this.token2.address);
    assert.equal(
      (await this.hulk.balanceOf(bar)).valueOf().toString(),
      "32965"
    );
    assert.equal(
      (await this.token1Token2.balanceOf(this.maker.address))
    ...
    // Should also work for Hulk-ETH pair
    await this.hulk.transfer(this.hulkWETH.address, "100000", {
      from: minter
    });
    await this.weth.transfer(this.hulkWETH.address, "100000", {
      from: minter
    });
    await this.hulkWETH.sync();
    await this.hulk.transfer(this.hulkWETH.address, "10000000", {
      from: minter,
    });
    await this.weth.transfer(this.hulkWETH.address, "10000000", {
    ...
    await this.hulkWETH.mint(minter);
    assert.equal(
      (await this.hulkWETH.balanceOf(this.maker.address)).valueOf().toString(),
      "16537"
    );
    await this.maker.convert(this.hulk.address, this.weth.address);
    assert.equal(
      (await this.hulk.balanceOf(bar)).valueOf().toString(),
      "66249"
    );
