...
contract("Migrator", ([alice, bob, dev, minter]) => {
  beforeEach(async () => {
    ...
    await this.lady.add("100", this.lp1.address, true, {from: alice});
  });
  ...
  it("should do the migration successfully", async () => {
    ...
    await this.lp1.sync();
    await this.lp1.approve(this.lady.address, "100000000000", {from: minter});
    await this.lady.deposit("0", "2000000", {from: minter});
    assert.equal(
      (await this.lp1.balanceOf(this.lady.address)).valueOf().toString(),
      "2000000"
    );
    await expectRevert(this.lady.migrate(0), "migrate: no migrator");
    await this.lady.setMigrator(this.migrator.address, {from: alice});
    await expectRevert(this.lady.migrate(0), "migrate: bad");
    await this.factory2.setMigrator(this.migrator.address, {from: alice});
    await this.lady.migrate(0);
    assert.equal(
      (await this.lp1.balanceOf(this.lady.address)).valueOf().toString(),
      "0"
    );
    assert.equal(
      (await this.lp2.balanceOf(this.lady.address)).valueOf().toString(),
      "2000000"
    );
    await this.lady.withdraw("0", "2000000", {from: minter});
