...
contract("Migrator", ([alice, bob, dev, minter]) => {
  beforeEach(async () => {
    this.factory1 = await UniswapV2Factory.new(alice, {
      from: alice
    });
    this.factory2 = await UniswapV2Factory.new(alice, {
      from: alice
    });
  ...
  it("should do the migration successfully", async () => {
    ...
    assert.equal(
      (await this.lp1.balanceOf(minter)).valueOf().toString(),
      "2235067"
    );
    ...
    await this.lp1.sync();
    await this.lp1.approve(this.farmer.address, "100000000000", {
      from: minter
    });
    await this.farmer.deposit("0", "2000000", {
      from: minter
    });
    assert.equal(
      (await this.lp1.balanceOf(this.farmer.address)).valueOf().toString(),
      "2000000"
    );
    await expectRevert(this.farmer.migrate(0), "migrate: no migrator");
    await this.farmer.setMigrator(this.migrator.address, {
      from: alice
    });
    await expectRevert(this.farmer.migrate(0), "migrate: bad");
    await this.factory2.setMigrator(this.migrator.address, {
      from: alice
    });
    await this.farmer.migrate(0);
    assert.equal(
      (await this.lp1.balanceOf(this.farmer.address)).valueOf().toString(),
      "0"
    );
    assert.equal(
      (await this.lp2.balanceOf(this.farmer.address)).valueOf().toString(),
      "2000000"
    );
    await this.farmer.withdraw("0", "2000000", {
      from: minter
    });
    ...
    assert.equal(
      (await this.weth.balanceOf(bob)).valueOf().toString(),
      "451685"
    );
  });
