...
contract("Governor", ([alice, minter, dev]) => {
  it("should work", async () => {
    ...
    await this.lunch.delegate(dev, {
      from: dev,
    });
    this.lady = await LunchLady.new(this.lunch.address, dev, "100", "0", "0", {
      from: alice,
    });
    ...
    // Perform another deposit to make sure some LUNCHs are minted in that 1 block.
    await this.lady.deposit(0, "100", {
      from: minter,
    });
    assert.equal((await this.lunch.totalSupply()).valueOf().toString(), "110");
    assert.equal(
      (await this.lunch.balanceOf(minter)).valueOf().toString(),
      "100"
    );
    assert.equal((await this.lunch.balanceOf(dev)).valueOf().toString(), "10");
    ...
    await this.lady.transferOwnership(this.timelock.address, {
      from: alice,
    });
    await expectRevert(
      this.lady.add("100", this.lp2.address, true, {
        from: alice,
      }),
